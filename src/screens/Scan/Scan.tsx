...
export async function CheckLinkType(
  ...
  if (url.startsWith('openid4vp')) {
    if (!agent) {
      console.warn('⚠️ Agent not initialized yet')
      return ''
    }
    ...
    try {
      ...
      if (id) {
        navigation.navigate(TabStacks.PresentationStack as never, {
          screen: Screens.PresentationRequest,
  ...
  if (url.startsWith('otpauth://totp/')) {
    navigation.navigate(TabStacks.OtpStack as never, {
